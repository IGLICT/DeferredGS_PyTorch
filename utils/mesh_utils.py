...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            self.diffuse_maps.append(render_pkg['diffuse'].cpu())
            self.roughness_maps.append(render_pkg['roughness'].cpu())
            self.specular_maps.append(render_pkg['specular'].cpu())
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            if is_ORB_relight:
                if (viewpoint_cam.image_name == self.orb_image_name) and (self.orb_scene_name in viewpoint_cam.image_path):
                    ...
                    save_img_u8(self.rgbmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, self.orb_scene_name+'_'+self.orb_image_name + ".png"))
            else:
                # save_img_u8(gt.permute(1,2,0).cpu().numpy(), os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))
                save_img_u8(self.rgbmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
                save_img_u8(self.diffuse_maps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, '{0:05d}_diffuse'.format(idx) + ".png"))
                save_img_u8(self.roughness_maps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, '{0:05d}_roughness'.format(idx) + ".png"))
                save_img_u8(self.specular_maps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, '{0:05d}_specular'.format(idx) + ".png"))
